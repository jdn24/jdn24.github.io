...
$singlePage = get_page_by_path('single', OBJECT, 'wp_template');
$singlePageId = is_object($singlePage) ? $singlePage->ID : '';
if (!$singlePageId) {
    $templateContent = file_get_contents(get_template_directory() . '/templates/single.html');
    $singlePageId = wp_insert_post(array(
        'post_title' => 'Single',
        'post_name' => 'single',
        'post_type' => 'wp_template',
        'post_status' => 'publish',
        'post_content' => $templateContent // Set the content from the theme file
    ));
...
$single_template = file_get_contents(get_template_directory() . '/templates/single.html');
$single2_template = $single_template;
$single3_template = str_replace('<!-- wp:template-part {"slug":"titlepost","theme":"greenshift","tagName":"div","align":"wide","className":"site-title-area"} /-->', '<!-- wp:pattern {"slug":"greenshift/parts-titlepost-cover"} /-->', $single_template);
$single4_template = str_replace('<!-- wp:template-part {"slug":"titlepost","theme":"greenshift","tagName":"div","align":"wide","className":"site-title-area"} /-->', '<!-- wp:pattern {"slug":"greenshift/parts-titlepost-bg"} /-->', $single_template);
...
if (
    ...
    if (sanitize_text_field($_POST['single_type'])) {
        ...
        $onboard_options['single'] = sanitize_text_field($_POST['single_type']);
        update_option('greenshift_onboard_options', $onboard_options);
    }

    if (isset($_POST['gspb_save_settings_next'])) {
        echo '<script>window.location.replace("'.admin_url('admin.php?page=greenshift_theme_settings&tab=archive').'");</script>';
        exit;
    }
}
        ...
        <div class="settings-container">
            <div class="gs-box notice_type icon_type">
                ...
                <div class="gs-box-text">
                    <?php esc_html_e("Selecting single page template will overwrite your existing template. You can edit elements in", "greenshift"); ?>
                    <a href="<?php echo admin_url('/site-editor.php?postType=wp_template&postId=greenshift%2F%2Fsingle&canvas=edit'); ?>" target="_blank"><?php esc_html_e("Site Editor", "greenshift"); ?></a>
                </div>
            </div>
            <div>
                <h4>
                    <?php esc_html_e("Your single displays", "greenshift"); ?>
                </h4>
            </div>
            <div class="flex-right">
                <div style=" display: flex; flex-direction: column; gap: 15px">
                    <div class="template_item">
                        <input type="radio" name="single_type" id="single1" value="single1" onchange="handleRadioButtonChange(this)" data-pageid="<?php echo (int)$singlePageId; ?>" data-content='<?php echo htmlspecialchars(json_encode($templateContent), ENT_QUOTES); ?>' <?php echo ((!empty($onboard_options['single']) && $onboard_options['single'] == 'default') || empty($onboard_options['single'])) ? 'checked' : ''; ?>>
                        <label for="single1"><?php esc_html_e("Current single", "greenshift"); ?></label>
                    </div>
                    <div class="template_item">
                        <input type="radio" name="single_type" id="single2" value="single2" data-pageid="<?php echo (int)$singlePageId; ?>" data-content='<?php echo json_encode($single2_template); ?>' onchange="handleRadioButtonChange(this)" <?php echo (!empty($onboard_options['single']) && $onboard_options['single'] == 'single2') ? 'checked' : ''; ?>>
                        <label for="single2"><?php esc_html_e("Single Default", "greenshift"); ?></label>

                    </div>
                    <div class="template_item">
                        <input type="radio" id="single3" name="single_type" value="single3" data-pageid="<?php echo (int)$singlePageId; ?>" data-content='<?php echo json_encode($single3_template); ?>' onchange="handleRadioButtonChange(this)" <?php echo (!empty($onboard_options['single']) && $onboard_options['single'] == 'single3') ? 'checked' : ''; ?>>
                        <label for="single3"><?php esc_html_e("Single with Cover block", "greenshift"); ?></label>

                    </div>

                    <div class="template_item">
                        <input type="radio" id="single4" name="single_type" value="single4" data-pageid="<?php echo (int)$singlePageId; ?>" data-content='<?php echo json_encode($single4_template); ?>' onchange="handleRadioButtonChange(this)" <?php echo (!empty($onboard_options['single']) && $onboard_options['single'] == 'single4') ? 'checked' : ''; ?>>
                        <label for="single4"> <?php esc_html_e("Single with negative margin", "greenshift"); ?></label>
