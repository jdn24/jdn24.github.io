...
$latest_link = $shop_page_url = get_permalink( wc_get_page_id( 'shop' ) );

$archivePage = get_page_by_path('archive-product', OBJECT, 'wp_template');
$archivePageId = is_object($archivePage) ? $archivePage->ID : '';
if (!$archivePageId) {
    $templateContent = file_get_contents(get_template_directory() . '/templates/archive-product.html');
    $archivePageId = wp_insert_post(array(
        'post_title' => 'Shop Archive',
...
$archive_template = file_get_contents(get_template_directory() . '/templates/archive-product.html');
$archive2_template = $archive_template;
$archive3_template = '<!-- wp:template-part {"slug":"header","theme":"greenshift"} /--><!-- wp:pattern {"slug":"gspb_woo/template/shop"} /--><!-- wp:template-part {"slug":"footer","theme":"greenshift"} /-->';
...
if (
    ...
    if (isset($_POST['gspb_save_settings_next'])) {
        if(class_exists('woocommerce')){
            echo '<script>window.location.replace("'.admin_url('admin.php?page=greenshift_theme_settings&tab=searchproduct').'");</script>';
        }else{
        ...
        <div class="settings-container">
            <div class="gs-box notice_type icon_type">
                ...
                <div class="gs-box-text">
                    <?php esc_html_e("Selecting archive product template will overwrite your existing template. You can edit elements in", "greenshift"); ?>
                    <a href="<?php echo admin_url('/site-editor.php?postType=wp_template&postId=greenshift%2F%2Farchive-product&canvas=edit'); ?>" target="_blank"><?php esc_html_e("Site Editor", "greenshift"); ?></a>
                </div>
            </div>
            ...
            <div class="flex-right">
                <div style=" display: flex; flex-direction: column; gap: 15px">
                    ...
                    <div class="template_item">
                        ...
                        <label for="archive2"><?php esc_html_e("Shop Default", "greenshift"); ?></label>
                    ...
                    <div class="template_item">
                        ...
                        <label for="archive3"><?php esc_html_e("Improved Shop page", "greenshift"); ?></label>
                    ...
                    <div class="gspb_main_buttonss">
                            <div id="gspb_template_import_btnss" data-type="archiveproduct" data-id="<?php echo (int)$archivePageId;?>" data-tab="Templates"></div>
                        </div>
