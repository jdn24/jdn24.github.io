...
if ($latest_posts) {
    foreach ($latest_posts as $post) {
        ...
        $latest_link = get_permalink($latest_post_id);
    }
}

$singlePage = get_page_by_path('single-product', OBJECT, 'wp_template');
$singlePageId = is_object($singlePage) ? $singlePage->ID : '';
if (!$singlePageId) {
    $templateContent = file_get_contents(get_template_directory() . '/templates/single-product.html');
    $singlePageId = wp_insert_post(array(
    ...
    wp_set_object_terms($singlePageId, $category_slug, $category_domain, false);
} else {
    $templateContent = get_post_field('post_content', $singlePageId);
}

$single_template = file_get_contents(get_template_directory() . '/templates/single-product.html');
$single2_template = $single_template;
$single3_template = '<!-- wp:template-part {"slug":"header","theme":"greenshift"} /--><!-- wp:pattern {"slug":"gspb_woo/template/single"} /--><!-- wp:template-part {"slug":"footer","theme":"greenshift"} /-->';
$single4_template = '<!-- wp:template-part {"slug":"header","theme":"greenshift"} /--><!-- wp:pattern {"slug":"gspb_woo/template/singlegallery"} /--><!-- wp:template-part {"slug":"footer","theme":"greenshift"} /-->';
...
if (
    ...
    if (sanitize_text_field($_POST['single_type'])) {
        if (empty($onboard_options)) {
            $onboard_options = array();
        }
        $onboard_options['singleproduct'] = sanitize_text_field($_POST['single_type']);
        update_option('greenshift_onboard_options', $onboard_options);
    }

    if (isset($_POST['gspb_save_settings_next'])) {
        echo '<script>window.location.replace("'.admin_url('admin.php?page=greenshift_theme_settings&tab=archiveproduct').'");</script>';
        exit;
    }
}
            ...
            <div>
                ...
                <?php if (!defined('GREENSHIFTWOO_DIR_URL') || !defined('GREENSHIFTQUERY_DIR_URL')) : ?>
                    <div class="gs-box notice_type icon_type">
                        ...
                        <div class="gs-box-text">
                            <?php esc_html_e("This option requires to have Greenshift Query and Woocommerce addon", "greenshift"); ?>
                            <a href="<?php echo admin_url('admin.php?page=greenshift_dashboard-addons'); ?>" class="button button-primary"><?php esc_html_e("Check your Addons", "greenshift"); ?></a>
                            <a href="<?php echo esc_url('https://shop.greenshiftwp.com/downloads/woocommerce-pack/'); ?>" target="_blank" class="button button-primary"><?php esc_html_e("Buy Woocommerce plan (includes Woo and Query addon)", "greenshift"); ?></a>
                        </div>
                    </div>
                <?php endif; ?>
                <h4>
                    <?php esc_html_e("Your single product displays", "greenshift"); ?>
                </h4>
            </div>
            <div class="flex-right">
                <div style=" display: flex; flex-direction: column; gap: 15px">
                    <div class="template_item">
                        <input type="radio" name="single_type" id="single1" value="single1" onchange="handleRadioButtonChange(this)" data-pageid="<?php echo (int)$singlePageId; ?>" data-content='<?php echo htmlspecialchars(json_encode($templateContent), ENT_QUOTES); ?>' <?php echo ((!empty($onboard_options['singleproduct']) && $onboard_options['singleproduct'] == 'default') || empty($onboard_options['singleproduct'])) ? 'checked' : ''; ?>>
                        <label for="single1"><?php esc_html_e("Current single", "greenshift"); ?></label>
                    </div>
                    <div class="template_item">
                        <input type="radio" name="single_type" id="single2" value="single2" data-pageid="<?php echo (int)$singlePageId; ?>" data-content='<?php echo json_encode($single2_template); ?>' onchange="handleRadioButtonChange(this)" <?php echo (!empty($onboard_options['singleproduct']) && $onboard_options['singleproduct'] == 'single2') ? 'checked' : ''; ?>>
                        <label for="single2"><?php esc_html_e("Single Default", "greenshift"); ?></label>

                    </div>
                    <div class="template_item">
                        <input type="radio" id="single3" name="single_type" value="single3" data-pageid="<?php echo (int)$singlePageId; ?>" data-content='<?php echo json_encode($single3_template); ?>' onchange="handleRadioButtonChange(this)" <?php echo (!empty($onboard_options['singleproduct']) && $onboard_options['singleproduct'] == 'single3') ? 'checked' : ''; ?>>
                        <label for="single3"><?php esc_html_e("Single Improved", "greenshift"); ?></label>

                    </div>

                    <div class="template_item">
                        <input type="radio" id="single4" name="single_type" value="single4" data-pageid="<?php echo (int)$singlePageId; ?>" data-content='<?php echo json_encode($single4_template); ?>' onchange="handleRadioButtonChange(this)" <?php echo (!empty($onboard_options['singleproduct']) && $onboard_options['singleproduct'] == 'single4') ? 'checked' : ''; ?>>
                        <label for="single4"> <?php esc_html_e("Single with 2 column gallery", "greenshift"); ?></label>

                    </div>


                    <div class="gspb_main_buttons">
                        <div id="gspb_template_import_btns" data-type="singleproduct" data-id="<?php echo (int)$singlePageId; ?>" data-tab="Templates" data-key-default="115"></div>
                    </div>
